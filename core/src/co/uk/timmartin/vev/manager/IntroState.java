  ...
  public void addParticles() {
    ...
    for (double a = -Math.PI; a <= Math.PI; a += (2 * Math.PI) / (double) (Particle.values().length - 1)) { // -1 due to kBlank
      ...
      for (int i = 0; i < 200; ++i) {
        ...
        s.pathTo(Sprite.findPathingLocation(genTile, false, true, true, true), null, null);
...
//    introTileStage is static
  ...
  public Stage getIntroWarpStage() {
    return introWarpStage;
  }

  public Stage getIntroBuildingStage() {
    return introBuildingStage;
  }

  public Stage getIntroGeneratingStage() {
    return introGeneratingStage;
  }
